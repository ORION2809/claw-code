# TUI Enhancement Plan — Claw Code (`claw-cli`)
...
|---|---|---|---|
| `claw-cli` | Main binary: REPL loop, arg parsing, rendering, API bridge | ~3,600 | **Primary TUI surface** |
| `runtime` | Session, conversation loop, config, permissions, compaction | ~5,300 | Provides data/state |
| `api` | API HTTP client + SSE streaming | ~1,500 | Provides stream events |
...
| 0.4 | **Create a `tui/` module** — Introduce `crates/claw-cli/src/tui/mod.rs` as the namespace for all new TUI components: `status_bar.rs`, `layout.rs`, `tool_panel.rs`, etc. | S |
