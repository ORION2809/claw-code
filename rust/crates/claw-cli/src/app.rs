...
use crate::args::{
    parse_cli_action, render_completion_script, render_manpage, AllowedToolSet, CliAction,
    CliOutputFormat,
};
use crate::format::{
...
use crate::input;
use crate::render::{normalize_theme_name, MarkdownStreamState, Spinner, TerminalRenderer};
...
use commands::{
    handle_agents_slash_command, handle_doctor_slash_command, handle_hooks_slash_command,
...
use serde_json::json;
use server::AppState as HttpAppState;
use tools::{execute_tool, GlobalToolRegistry};
...
const TASK_TAIL_POLL_INTERVAL: Duration = Duration::from_millis(250);
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_cli_action(&args, normalize_allowed_tools)? {
        CliAction::DumpManifests => dump_manifests(),
        CliAction::BootstrapPlan => print_bootstrap_plan(),
        CliAction::Agents { args } => LiveCli::print_agents(args.as_deref())?,
        CliAction::Skills { args } => LiveCli::print_skills(args.as_deref())?,
        CliAction::GenerateCompletions { shell, output } => {
            write_generated_artifact(output, &render_completion_script(shell), "shell completion")?;
        }
        CliAction::GenerateManpage { output } => {
            write_generated_artifact(output, &render_manpage(), "man page")?;
        }
        ...
        CliAction::Pipe {
            ...
            allowed_tools,
            permission_mode,
        } => run_pipe(model, output_format, allowed_tools, permission_mode)?,
        CliAction::Login => run_login()?,
        CliAction::Logout => run_logout()?,
        CliAction::Init => run_init()?,
        CliAction::Serve { host, port } => run_server(&host, port)?,
        CliAction::Repl {
            model,
            allowed_tools,
            permission_mode,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    parse_cli_action(args.iter().cloned(), normalize_allowed_tools)
}
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Context => {
            ...
            Ok(ResumeCommandOutcome {
                ...
                message: Some(format!(
                    ...
                    session_path.display(),
                    session.messages.len(),
                    runtime::estimate_session_tokens(session),
        ...
        SlashCommand::Summary => {
            ...
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(if result.formatted_summary.trim().is_empty() {
                    format!(
                        "Summary\n  Result           insufficient history to summarize\n  Messages         {}",
                        session.messages.len()
                    )
                } else {
                    format!("Summary\n{}", result.formatted_summary.trim())
                }),
            })
        }
        SlashCommand::Stats => {
        ...
        SlashCommand::Rewind { steps } => {
            let steps = steps
                ...
                .unwrap_or("1")
                .parse::<usize>()
                .map_err(|_| "rewind steps must be a positive integer")?;
            let mut updated = session.clone();
            let new_len = updated.messages.len().saturating_sub(steps);
            updated.messages.truncate(new_len);
            updated.save_to_path(session_path)?;
            Ok(ResumeCommandOutcome {
                session: updated,
                message: Some(format!(
                    "Rewind\n  Removed messages {steps}\n  Messages kept    {new_len}"
                )),
            })
        }
        ...
        SlashCommand::Hooks => {
            ...
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(handle_hooks_slash_command(&cwd)?),
            })
        }
        SlashCommand::Mcp { action, .. } => {
            ...
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(handle_mcp_slash_command(action.as_deref(), &cwd)?),
            })
        }
        SlashCommand::Doctor => {
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(handle_doctor_slash_command(&cwd)?),
            })
        }
        SlashCommand::Tag { .. }
        | SlashCommand::Tasks { .. }
...
fn run_repl(
    ...
    let mut vim_enabled = configured_vim_enabled();
    editor.set_vim_enabled(vim_enabled);
    println!("{}", cli.startup_banner());

    loop {
        let outcome = editor.read_line()?;
        if editor.vim_enabled() != vim_enabled {
            vim_enabled = editor.vim_enabled();
            persist_vim_enabled(vim_enabled)?;
        }
...
fn run_pipe(
    ...
    if prompt.is_empty() {
        return Err("pipe mode requires non-empty stdin input".into());
    }
    LiveCli::new(model, true, allowed_tools, permission_mode)?
        .run_turn_with_output(prompt, output_format)
}
...
fn run_server(host: &str, port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Server\n  Listening        http://{address}\n  Sessions API     POST /sessions\n  Stream API       GET  /sessions/{{id}}/events"
    );
    let runtime = tokio::runtime::Runtime::new()?;
    runtime.block_on(async move {
        let listener = tokio::net::TcpListener::bind(&address).await?;
        server::serve(listener, HttpAppState::default()).await
    })?;
    Ok(())
}
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let workspace_summary = git_branch.as_deref().map_or_else(
            ...
            |branch| format!("{workspace_name} Ã‚Â· {branch}"),
        );
        let has_claw_md = cwd
            .as_ref()
        ...
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;45mÃ°Å¸Â¦Å¾ Claw Code\x1b[0m"
                } else {
                    "Claw Code"
                },
                if color {
                    "\x1b[2mÃ‚Â· ready\x1b[0m"
                } else {
                    "Ã‚Â· ready"
                }
            ),
            ...
            format!(
                "  Quick start      {}",
                if has_claw_md {
                    "/help Ã‚Â· /status Ã‚Â· ask for a task"
                } else {
                    "/init Ã‚Â· /help Ã‚Â· /status"
                }
            ),
            "  Editor           Tab completes slash commands Ã‚Â· /vim toggles modal editing"
                .to_string(),
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let renderer = configured_terminal_renderer();
        spinner.tick("Ã°Å¸Â¦â‚¬ Thinking...", renderer.color_theme(), &mut stdout)?;
        ...
        match result {
            Ok(summary) => {
                spinner.finish("Ã¢Å“Â¨ Done", renderer.color_theme(), &mut stdout)?;
                println!();
            ...
            Err(error) => {
                spinner.fail("Ã¢ÂÅ’ Request failed", renderer.color_theme(), &mut stdout)?;
                Err(Box::new(error))
            }
        }
    }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Share { path } => {
                self.share_session(path.as_deref())?;
                false
            }
            SlashCommand::Rewind { steps } => self.rewind_session(steps.as_deref())?,
            SlashCommand::Tag { name } => {
                self.add_session_tag(name.as_deref())?;
                false
            }
            ...
            SlashCommand::ReloadPlugins => {
                ...
                println!("Plugins\n  Result           reloaded runtime features");
                false
            }
            SlashCommand::Mcp { action, .. } => {
                Self::print_mcp(action.as_deref())?;
                false
            }
            ...
            SlashCommand::SecurityReview { scope } => {
                self.run_security_review(scope.as_deref())?;
                false
            }
            SlashCommand::Doctor => {
                Self::print_doctor()?;
            ...
            SlashCommand::Tasks { action, target } => {
                Self::print_tasks(action.as_deref(), target.as_deref())?;
                false
            }
            SlashCommand::Login => {
                run_login()?;
            ...
            SlashCommand::SandboxToggle => self.toggle_sandbox_mode()?,
            SlashCommand::Agents { args } => {
            ...
            SlashCommand::AddDir { path } => {
                Self::add_workspace_dir(path.as_deref())?;
                false
            }
            SlashCommand::Voice { args } => {
                Self::record_voice_preference(args.as_deref())?;
                false
            }
            SlashCommand::Stickers { name } => {
                Self::print_sticker(name.as_deref());
                false
            }
            SlashCommand::Branch { .. } => {
                eprintln!(
    ...
    fn print_status(&self) {
        ...
        let _ = tui::print_report(&report);
    }

    fn print_context(&self) -> Result<(), Box<dyn std::error::Error>> {
        let context = status_context(Some(&self.session.path))?;
        print_report(&format!(
            "Context\n  System prompts    {}\n  Prompt chars      {}\n  Estimated tokens  {}\n  Session messages  {}\n  Instruction files {}\n  Config files      {}/{}\n  Project root      {}\n  Session file      {}",
            self.system_prompt.len(),
            self.system_prompt.iter().map(String::len).sum::<usize>(),
    ...
    fn toggle_sandbox_mode(&mut self) -> Result<bool, Box<dyn std::error::Error>> {
        let next = match self.permission_mode {
            PermissionMode::WorkspaceWrite
            | PermissionMode::DangerFullAccess
            | PermissionMode::Allow => "read-only",
            PermissionMode::ReadOnly | PermissionMode::Prompt => "workspace-write",
        };
        self.set_permissions(Some(next.to_string()))
    }
    ...
    fn print_config(section: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        print_report(&render_config_report(section)?)?;
        Ok(())
    }

    fn print_memory() -> Result<(), Box<dyn std::error::Error>> {
        print_report(&render_memory_report()?)?;
        Ok(())
    }

    fn print_agents(args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        print_report(&handle_agents_slash_command(args, &cwd)?)?;
        Ok(())
    }

    fn print_skills(args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        print_report(&handle_skills_slash_command(args, &cwd)?)?;
        Ok(())
    }

    fn print_hooks() -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        print_report(&handle_hooks_slash_command(&cwd)?)?;
        Ok(())
    }

    fn print_mcp(action: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        print_report(&handle_mcp_slash_command(action, &cwd)?)?;
        Ok(())
    }

    fn print_doctor() -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        print_report(&handle_doctor_slash_command(&cwd)?)?;
        Ok(())
    }
    ...
    fn print_tasks(
        ...
        let requested_action = action.unwrap_or("list").trim();
        let payload = match requested_action {
            "list" => {
                let input = target.map_or_else(|| json!({}), |value| json!({ "status": value }));
                print_report(&execute_tool("TaskList", &input)?)?;
                return Ok(());
            }
            "get" | "show" => {
                let task_id = require_task_id(target, requested_action)?;
                ("TaskGet", json!({ "taskId": task_id }))
            }
            "output" => {
                let task_id = require_task_id(target, requested_action)?;
                Self::print_task_output(task_id)?;
                return Ok(());
            }
            "tail" | "logs" => {
                let task_id = require_task_id(target, requested_action)?;
                Self::tail_task_output(task_id)?;
                return Ok(());
            }
            "pause" => {
                let task_id = require_task_id(target, requested_action)?;
                ("TaskPause", json!({ "taskId": task_id }))
            }
            "resume" => {
                let task_id = require_task_id(target, requested_action)?;
                ("TaskResume", json!({ "taskId": task_id }))
            }
            "stop" | "cancel" => {
                let task_id = require_task_id(target, requested_action)?;
                ("TaskStop", json!({ "taskId": task_id }))
            }
            "cleanup" => {
                let payload = build_task_cleanup_payload(target);
                ("TaskCleanup", payload)
            }
            "create" => {
                let prompt = target
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
                    .ok_or_else(|| render_tasks_usage().to_string())?;
                (
                    "TaskCreate",
                    json!({
                        "description": prompt,
            ...
            "update" => {
                let payload = build_task_update_payload(target)?;
                ("TaskUpdate", payload)
            }
            other if target.is_none() => {
                print_report(&execute_tool("TaskList", &json!({ "status": other }))?)?;
                return Ok(());
            }
            _ => return Err(render_tasks_usage().into()),
        };

        print_report(&execute_tool(payload.0, &payload.1)?)?;
        Ok(())
    }

    fn print_task_output(task_id: &str) -> Result<(), Box<dyn std::error::Error>> {
        let report = load_task_output(task_id)?;
        ...
        let body = if report.output.ends_with('\n') {
            report.output
        } else {
            format!("{}\n", report.output)
        };
        print_report(&format!(
            "Task output\n  Task             {}\n  Status           {}\n\n{}",
            report.task_id, report.status, body
        ))?;
        Ok(())
    }

    fn tail_task_output(task_id: &str) -> Result<(), Box<dyn std::error::Error>> {
        let mut task = load_task_record(task_id)?;
        let output_path = PathBuf::from(&task.output_file);
        ...
        loop {
            ...
            if let Some(delta) = contents.strip_prefix(&previous) {
                if delta.is_empty() {
                    idle_polls += 1;
                } else {
                    print!("{delta}");
            ...
            if !task_status_is_active(&task.status) && idle_polls > 1 {
            ...
            thread::sleep(TASK_TAIL_POLL_INTERVAL);
        }
    ...
    fn print_theme(name: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(name) = name {
            let normalized = normalize_theme_name(name).ok_or_else(|| {
                format!(
                    "unsupported theme '{name}'. Available themes: {}",
                    TerminalRenderer::supported_theme_names().join(", ")
                )
            })?;
            update_local_settings(|document| {
                document.insert(
                    "theme".to_string(),
        ...
        let current = configured_theme_name().unwrap_or_else(|| "dark".to_string());
        println!(
            "Theme\n  Current          {current}\n  Available        {}",
            TerminalRenderer::supported_theme_names().join(", ")
        );
    ...
    fn add_workspace_dir(path: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        update_local_settings(|document| {
            let value = document
                .entry("workspaceDirs".to_string())
                .or_insert_with(|| serde_json::Value::Array(Vec::new()));
            let array = value
                .as_array_mut()
                .ok_or_else(|| "workspaceDirs must be an array".to_string())?;
            if !array.iter().any(|entry| entry.as_str() == Some(path)) {
                array.push(serde_json::Value::String(path.to_string()));
            }
            Ok(())
        })?;
    ...
    fn record_voice_preference(args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let mode = match args.unwrap_or("on").trim() {
            "on" | "enabled" => "on",
            "off" | "disabled" => "off",
            other => {
                return Err(
                    format!("unsupported voice mode '{other}'. Use /voice [on|off].").into(),
                );
            }
        };
        update_local_settings(|document| {
    ...
    fn print_sticker(name: Option<&str>) {
        let sticker = match name.unwrap_or("ship").trim().to_ascii_lowercase().as_str() {
            "bug" => "[bug-found]",
            "review" => "[review-mode]",
            "focus" => "[deep-focus]",
            _ => "[ship-it]",
    ...
    fn print_diff() -> Result<(), Box<dyn std::error::Error>> {
        print_report(&render_diff_report()?)?;
        Ok(())
    }
    ...
    fn share_session(
        ...
        let path = requested_path.map_or_else(
            || format!("share-{}.txt", self.session.id),
            ToOwned::to_owned,
        );
        self.export_session(Some(&path))
    }

    fn rewind_session(&mut self, steps: Option<&str>) -> Result<bool, Box<dyn std::error::Error>> {
        let steps = steps
            .unwrap_or("1")
            .trim()
            .parse::<usize>()
        ...
        let new_len = original_len.saturating_sub(steps);
    ...
    fn add_session_tag(&self, name: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let Some(tag) = name.filter(|value| !value.trim().is_empty()) else {
            println!("Usage: /tag <tag>");
            return Ok(());
        };
        let tags_path = managed_state_dir()?.join("tags.json");
        let mut document = read_state_map(&tags_path)?;
        let entry = document
            .entry(self.session.id.clone())
            .or_insert_with(|| serde_json::Value::Array(Vec::new()));
        let tags = entry
            .as_array_mut()
            .ok_or_else(|| "session tag entry must be an array".to_string())?;
        if !tags.iter().any(|value| value.as_str() == Some(tag)) {
            tags.push(serde_json::Value::String(tag.to_string()));
        }
        write_state_map(&tags_path, &document)?;
        println!(
            "Tag\n  Session          {}\n  Added            {tag}",
            self.session.id
        );
    ...
    fn run_review(&self, scope: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let scope = scope.unwrap_or("the current workspace changes");
        let diff_summary = git_output(&["diff", "--stat"]).unwrap_or_default();
        let prompt = format!(
            "You are /review. Review {scope} with a code review mindset. Prioritize bugs, regressions, missing tests, and concrete risks. Start with findings, include file references when possible, and keep summaries brief.\n\nWorkspace diff summary:\n{}",
            truncate_for_prompt(&diff_summary, 10_000)
        );
        println!("{}", self.run_internal_prompt_text(&prompt, true)?);
        Ok(())
    }

    fn run_security_review(&self, scope: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let scope = scope.unwrap_or("the current workspace changes");
        let diff_summary = git_output(&["diff", "--stat"]).unwrap_or_default();
        let prompt = format!(
            "You are /security-review. Review {scope} with a security engineering mindset. Prioritize auth issues, sandbox escapes, unsafe command execution, credential leaks, secret handling, path traversal, injection vectors, and missing security tests. Start with concrete findings, include file references when possible, and keep summaries brief.\n\nWorkspace diff summary:\n{}",
            truncate_for_prompt(&diff_summary, 10_000)
        );
        println!("{}", self.run_internal_prompt_text(&prompt, true)?);
        Ok(())
...
fn render_session_list(active_session_id: &str) -> Result<String, Box<dyn std::error::Error>> {
    crate::session_mgr::render_session_list(active_session_id)
}

fn managed_state_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let path = env::current_dir()?.join(".claw");
...
fn update_local_settings<F>(mutate: F) -> Result<(), Box<dyn std::error::Error>>
where
    F: FnOnce(&mut serde_json::Map<String, serde_json::Value>) -> Result<(), String>,
{
    let path = managed_state_dir()?.join("settings.local.json");
    let mut document = read_state_map(&path)?;
    mutate(&mut document).map_err(|error| -> Box<dyn std::error::Error> { error.into() })?;
    write_state_map(&path, &document)?;
    Ok(())
}

fn configured_vim_enabled() -> bool {
    let path = match managed_state_dir() {
        Ok(dir) => dir.join("settings.local.json"),
        Err(_) => return false,
    };
    let Ok(document) = read_state_map(&path) else {
        return false;
    };
    document
        .get("vimMode")
...
fn persist_vim_enabled(enabled: bool) -> Result<(), Box<dyn std::error::Error>> {
    update_local_settings(|document| {
        document.insert("vimMode".to_string(), serde_json::Value::Bool(enabled));
        Ok(())
    })
}

fn configured_theme_name() -> Option<String> {
    let path = managed_state_dir().ok()?.join("settings.local.json");
    let document = read_state_map(&path).ok()?;
    document
        .get("theme")
...
fn configured_terminal_renderer() -> TerminalRenderer {
    let theme = configured_theme_name();
    TerminalRenderer::with_theme_name(theme.as_deref())
}

fn render_tasks_usage() -> &'static str {
    "Tasks\n  Usage            /tasks [list [status]|create <prompt>|show <id>|output <id>|tail <id>|pause <id>|resume <id>|stop <id>|cleanup [all|completed|failed|canceled|<id>]|update <id> --name <name> --description <desc> --model <model>]"
}
...
fn build_task_update_payload(
    target: Option<&str>,
) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    let Some(raw) = target.map(str::trim).filter(|value| !value.is_empty()) else {
        return Err(format!(
            "{}\n  Missing          task id and update fields",
            render_tasks_usage()
        )
        .into());
    };
    let tokens = raw.split_whitespace().collect::<Vec<_>>();
    let Some(task_id) = tokens.first() else {
        return Err(render_tasks_usage().into());
    };

    let mut index = 1;
    let mut payload = json!({ "taskId": task_id });
    let Some(object) = payload.as_object_mut() else {
        return Err("failed to build task update payload".into());
    };

    while index < tokens.len() {
        let flag = tokens[index];
        if !flag.starts_with("--") {
            return Err(format!("{}\n  Unexpected       {flag}", render_tasks_usage()).into());
        }
        ...
        let value = tokens[value_start..index].join(" ");
        if value.is_empty() {
            return Err(format!(
        ...
        match key {
            ...
            other => {
                return Err(format!(
                    "{}\n  Unsupported      --{other} (expected --name, --description, or --model)",
                    render_tasks_usage()
                )
                .into());
            }
    ...
    if object.len() == 1 {
        return Err(format!(
            "{}\n  Missing          at least one update field",
            render_tasks_usage()
        )
        .into());
    }
...
fn build_task_cleanup_payload(target: Option<&str>) -> serde_json::Value {
    let Some(raw) = target.map(str::trim).filter(|value| !value.is_empty()) else {
        return json!({});
    };
    ...
    match raw {
        "all" => json!({ "all": true }),
        "completed" | "failed" | "canceled" => json!({ "status": raw }),
        _ => json!({ "taskId": raw }),
    }
}

fn load_task_record(task_id: &str) -> Result<CliTaskRecord, Box<dyn std::error::Error>> {
    let response = execute_tool("TaskGet", &json!({ "taskId": task_id }))?;
    let value = serde_json::from_str::<serde_json::Value>(&response)?;
    Ok(CliTaskRecord {
        task_id: json_required_string(&value, "taskId")?.to_string(),
        status: json_required_string(&value, "status")?.to_string(),
        output_file: json_required_string(&value, "outputFile")?.to_string(),
    })
}

fn load_task_output(task_id: &str) -> Result<CliTaskOutput, Box<dyn std::error::Error>> {
    let response = execute_tool("TaskOutput", &json!({ "taskId": task_id }))?;
    let value = serde_json::from_str::<serde_json::Value>(&response)?;
    Ok(CliTaskOutput {
        task_id: json_required_string(&value, "taskId")?.to_string(),
        status: json_required_string(&value, "status")?.to_string(),
        output: json_required_string(&value, "output")?.to_string(),
    })
}
...
fn json_required_string<'a>(
    ...
    value
        .get(key)
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| format!("missing string field `{key}` in task response").into())
}
...
fn print_report(report: &str) -> Result<(), Box<dyn std::error::Error>> {
    tui::print_report(report)?;
    Ok(())
}
...
fn truncate_for_prompt(value: &str, limit: usize) -> String {
    if value.chars().count() <= limit {
        ...
        format!("{}\nÃ¢â‚¬Â¦[truncated]", truncated.trim_end())
    }
}
...
fn format_internal_prompt_progress_line(
    ...
    let status = status_bits.join(" Ã‚Â· ");
    match event {
        InternalPromptProgressEvent::Started => {
            format!(
                "Ã°Å¸Â§Â­ {} status Ã‚Â· planning started Ã‚Â· {status}",
                snapshot.command_label
            )
        }
        InternalPromptProgressEvent::Update => {
            format!("Ã¢â‚¬Â¦ {} status Ã‚Â· {status}", snapshot.command_label)
        }
        InternalPromptProgressEvent::Heartbeat => format!(
            "Ã¢â‚¬Â¦ {} heartbeat Ã‚Â· {elapsed_seconds}s elapsed Ã‚Â· {status}",
            snapshot.command_label
        ),
        InternalPromptProgressEvent::Complete => format!(
            "Ã¢Å“â€ {} status Ã‚Â· completed Ã‚Â· {elapsed_seconds}s elapsed Ã‚Â· {} steps total",
            snapshot.command_label, snapshot.step
        ),
        InternalPromptProgressEvent::Failed => format!(
            "Ã¢Å“Ëœ {} status Ã‚Â· failed Ã‚Â· {elapsed_seconds}s elapsed Ã‚Â· {}",
            snapshot.command_label,
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    let detail = match name {
        ...
        "read_file" | "Read" => {
            ...
            format!("\x1b[2mÃ°Å¸â€œâ€ž Reading {path}Ã¢â‚¬Â¦\x1b[0m")
        }
        "write_file" | "Write" => {
            let path = extract_tool_path(&parsed);
            let lines = parsed
            ...
            format!("\x1b[1;32mÃ¢Å“ÂÃ¯Â¸Â Writing {path}\x1b[0m \x1b[2m({lines} lines)\x1b[0m")
        }
        "edit_file" | "Edit" => {
            let path = extract_tool_path(&parsed);
            ...
            let new_value = parsed
                ...
                .unwrap_or_default();
            format!(
                "\x1b[1;33mÃ°Å¸â€œÂ Editing {path}\x1b[0m{}",
                format_patch_preview(old_value, new_value)
                    .map(|preview| format!("\n{preview}"))
                    .unwrap_or_default()
            )
        }
        "glob_search" | "Glob" => format_search_start("Ã°Å¸â€Å½ Glob", &parsed),
        "grep_search" | "Grep" => format_search_start("Ã°Å¸â€Å½ Grep", &parsed),
        "web_search" | "WebSearch" => parsed
            .get("query")
            .and_then(|value| value.as_str())
            .unwrap_or("?")
    ...
    let border = "Ã¢â€â‚¬".repeat(name.len() + 8);
    format!(
        "\x1b[38;5;245mÃ¢â€¢Â­Ã¢â€â‚¬ \x1b[1;36m{name}\x1b[0;38;5;245m Ã¢â€â‚¬Ã¢â€¢Â®\x1b[0m\n\x1b[38;5;245mÃ¢â€â€š\x1b[0m {detail}\n\x1b[38;5;245mÃ¢â€¢Â°{border}Ã¢â€¢Â¯\x1b[0m"
    )
}

fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    let icon = if is_error {
        "\x1b[1;31mÃ¢Å“â€”\x1b[0m"
    } else {
        "\x1b[1;32mÃ¢Å“â€œ\x1b[0m"
    };
    if is_error {
...
const DISPLAY_TRUNCATION_NOTICE: &str =
    "\x1b[2mÃ¢â‚¬Â¦ output truncated for display; full result preserved in session.\x1b[0m";
...
fn format_read_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    format!(
        "{icon} \x1b[2mÃ°Å¸â€œâ€ž Read {path} (lines {}-{} of {})\x1b[0m\n{}",
        start_line,
...
fn format_write_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    let line_count = parsed
        ...
        .map_or(0, |content| content.lines().count());
    format!(
        "{icon} \x1b[1;32mÃ¢Å“ÂÃ¯Â¸Â {} {path}\x1b[0m \x1b[2m({line_count} lines)\x1b[0m",
...
fn format_edit_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    match preview {
        Some(preview) => {
            format!("{icon} \x1b[1;33mÃ°Å¸â€œÂ Edited {path}{suffix}\x1b[0m\n{preview}")
        }
        None => format!("{icon} \x1b[1;33mÃ°Å¸â€œÂ Edited {path}{suffix}\x1b[0m"),
    }
}
...
fn push_output_block(
    ...
    match block {
        OutputContentBlock::Text { text } => {
            if !text.is_empty() {
                let rendered = configured_terminal_renderer().markdown_to_ansi(&text);
                write!(out, "{rendered}")
                    .and_then(|()| out.flush())
                    .map_err(|error| RuntimeError::new(error.to_string()))?;
                events.push(AssistantEvent::TextDelta(text));
...
impl CliToolExecutor {
    fn new(
        ...
        Self {
            renderer: configured_terminal_renderer(),
            emit_output,
            allowed_tools,
            tool_registry,
        }
...
fn append_audit_log(tool_name: &str, input: &serde_json::Value, success: bool, output: &str) {
    let Ok(path) = managed_state_dir().map(|dir| dir.join("audit.log")) else {
        return;
    };
    ...
    let record = json!({
        ...
        "input": input,
        "outputPreview": truncate_for_summary(output, 240),
    });
    let _ = writeln!(file, "{record}");
}

fn secret_exposure_hint(value: &str) -> Option<&'static str> {
    let trimmed = value.trim();
    if trimmed.contains("-----BEGIN") && trimmed.contains("PRIVATE KEY-----") {
        return Some("private-key-material");
    }
    if trimmed.contains("AKIA") || trimmed.contains("ASIA") {
        return Some("aws-access-key");
    }
    if trimmed.contains("ghp_") || trimmed.contains("github_pat_") {
        return Some("github-token");
    }
    if trimmed.contains("sk-") || trimmed.contains("xoxb-") || trimmed.contains("xoxp-") {
...
fn warn_on_secret_like_output(tool_name: &str, output: &str) {
    if let Some(signal) = secret_exposure_hint(output) {
        eprintln!(
            "Security warning\n  Tool             {tool_name}\n  Signal           {signal}\n  Action           review the output before copying or sharing it"
        );
    }
}
...
impl ToolExecutor for CliToolExecutor {
    fn execute(&mut self, tool_name: &str, input: &str) -> Result<String, ToolError> {
        ...
        match self.tool_registry.execute(tool_name, &value) {
            Ok(output) => {
                append_audit_log(tool_name, &value, true, &output);
                warn_on_secret_like_output(tool_name, &output);
                if self.emit_output {
            ...
            Err(error) => {
                append_audit_log(tool_name, &value, false, &error);
                warn_on_secret_like_output(tool_name, &error);
                if self.emit_output {
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  claw --pipe [--model MODEL] [--output-format text|json]"
    )?;
    writeln!(
        out,
        "      Read one prompt from stdin and print the response"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  claw serve [--host HOST] [--port PORT]")?;
    writeln!(
        out,
        "  claw completions <bash|zsh|fish|powershell|elvish> [--output PATH]"
    )?;
    writeln!(out, "  claw manpage [--output PATH]")?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  --pipe                     Read a single prompt from stdin"
    )?;
    writeln!(
        out,
...
fn print_help() {
    ...
    if print_help_to(&mut output).is_ok() {
        if let Ok(report) = String::from_utf8(output) {
            let _ = tui::print_report(report.trim_end());
        }
    }
}
...
mod tests {
    use super::{
        build_task_cleanup_payload, build_task_update_payload, describe_tool_progress,
        ...
        resume_supported_slash_commands, secret_exposure_hint, status_context,
        suggest_repl_commands, task_status_is_active, CliAction, CliOutputFormat,
    ...
    #[test]
    fn builds_task_update_payload_from_flagged_repl_args() {
        let payload = build_task_update_payload(Some(
            "task-123 --name reviewer --description investigate flaky tests --model gpt-5.4",
        ))
        .expect("task update payload should parse");
    ...
    #[test]
    fn task_update_payload_requires_update_fields() {
        let error = build_task_update_payload(Some("task-123"))
            .expect_err("missing update fields should fail");
        assert!(error.to_string().contains("Missing"));
    }

    #[test]
    fn builds_task_cleanup_payload_for_common_targets() {
        assert_eq!(build_task_cleanup_payload(None), json!({}));
        assert_eq!(
            build_task_cleanup_payload(Some("all")),
            json!({ "all": true })
        );
        assert_eq!(
            build_task_cleanup_payload(Some("completed")),
            json!({ "status": "completed" })
        );
        assert_eq!(
            build_task_cleanup_payload(Some("task-123")),
            json!({ "taskId": "task-123" })
        );
    }

    #[test]
    fn active_task_statuses_cover_running_states_only() {
        assert!(task_status_is_active("pending"));
        assert!(task_status_is_active("running"));
        assert!(task_status_is_active("pause_requested"));
        assert!(task_status_is_active("paused"));
        assert!(task_status_is_active("cancel_requested"));
        assert!(!task_status_is_active("completed"));
        assert!(!task_status_is_active("failed"));
    }
    ...
    fn resume_supported_command_list_matches_expected_surface() {
        ...
        assert_eq!(
            names,
            vec![
                "help", "status", "compact", "clear", "cost", "config", "memory", "context",
                "init", "diff", "version", "export", "summary", "stats", "share", "rewind", "tag",
                "hooks", "mcp", "doctor", "tasks", "theme", "agents", "skills", "add-dir", "voice",
                "stickers",
            ]
        );
    }
 ...
 M src/main.rs",
    ...
    fn tool_rendering_helpers_compact_output() {
        ...
        assert!(done.contains("Ã°Å¸â€œâ€ž Read src/main.rs"));
    ...
    #[test]
    fn secret_exposure_hint_detects_common_tokens() {
        assert_eq!(
            secret_exposure_hint("Authorization: Bearer sk-test-123"),
            Some("api-token")
        );
        assert_eq!(
            secret_exposure_hint("-----BEGIN PRIVATE KEY-----"),
            Some("private-key-material")
        );
        assert_eq!(secret_exposure_hint("normal output"), None);
    }
}
